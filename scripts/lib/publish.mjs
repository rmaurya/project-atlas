...
${/*
  * **Here the value IS baked, and that is not a contradiction.**
  *
  * On the generated site the footer reads `build-stamp.txt` at load, because writing a time into the HTML
  * would break the byte-identical rebuild. A bundle has no such property to protect and no file to read: it
  * is one detached document, it already stamps `generatedAt` into the snapshot bar above, and it is
  * *specifically* the artifact where not knowing the age costs the reader most. So the moment of export is
  * written into the attribute, and the same wire the pages carry renders it into both zones.
  *
  * With no `generatedAt` the attribute is absent and the wire — which sees `__ATLAS_SNAPSHOT__` and knows
  * there is nothing to fetch — says "not recorded" rather than guessing.
  */''}<p class="builtat" id="builtAt"${about.generatedAt ? ` data-built="${escapeHtml(about.generatedAt)}"` : ''} hidden></p></footer>
...
// Page chrome, not page content: these ids repeat on every page by design, so they are not collisions to be
// renamed. `builtAt` joins them because the bundle keeps exactly one footer and every page's script looks for
// it by that name — renamed per page, each copy would address an element that does not exist and the bundle
// would ship a footer that never fills in.
const CHROME_IDS = new Set(['themeToggle', 'clock', 'builtAt']);
...
export function exportSingleFile(root, cfg, which = 'dashboard') {
  ...
  /*
   * **The poller is switched off by the flag, not by a regex — and this one had already rotted.**
   *
   * `exportBundle` learned this the hard way and wrote it down: cutting the poller out with a pattern that
   * encodes the punctuation of a function it does not own matches nothing the day that function is touched,
   * and the export reports success while shipping the live poller anyway. That is exactly what had happened
   * here. The pattern this replaces was `poll(); setInterval(poll, \d+);`; the call site became
   * `poll(); timer = setInterval(poll, STEPS[0]);` when polling grew a back-off, and this line has matched
   * nothing since. A standalone file handed to someone has been requesting `build-stamp.txt` against
   * whatever directory it was opened from, three times, on every open.
   *
   * The same flag the bundle uses, in the same place — first thing in the body, ahead of every script the
   * page carries — so the poller reads it, and so the footer's build stamp knows not to fetch either.
   * Neither can half-work: either the flag is read, or the assertions about it fail.
   */
