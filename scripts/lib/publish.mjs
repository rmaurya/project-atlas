 ...
 */
/**
 * Names docs-atlas generates itself. A source document must never be allowed to claim one: an early version
...
export function buildWikiPages(index, health, plan, cfg, root = null) {
  ...
  const banner = (src) => {
    ...
    return `> **Generated page — do not edit here.** This wiki is written by \`docs-atlas\` from the repository.\n` +
  ...
  pages.set('_Footer', `_Generated by docs-atlas from ${index.stats.documents} source documents. Do not edit here._`);
...
export function stageWiki(root, cfg, built, { push = false, force = false, importDrift = false } = {}) {
  ...
  if (prior) {
    ...
    drift.push({ page: '(entire wiki)', source: null, kind: 'unmanaged',
      detail: 'The wiki has pages but no manifest — it was not written by docs-atlas.' });
  }
  ...
  const manifest = { generatedBy: 'docs-atlas', source: slug, pages: {} };
  ...
  if (push) {
    ...
    try {
      execFileSync('git', ['-c', 'user.email=atlas@local', '-c', 'user.name=docs-atlas',
        'commit', '-qm', `docs-atlas: publish ${built.pages.size} page(s)`], { cwd: work, stdio: 'ignore' });
...
export function stagePages(root, cfg, { push = false } = {}) {
  ...
  if (push) {
    ...
    execFileSync('git', ['add', '-A'], { cwd: work, stdio: 'ignore' });
    execFileSync('git', ['-c', 'user.email=atlas@local', '-c', 'user.name=docs-atlas',
      'commit', '-qm', 'docs-atlas: publish site'], { cwd: work, stdio: 'ignore' });
