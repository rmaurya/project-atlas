...
owes an answer to and marks the result a stub until a human deletes the marker (`scripts/atlas.mjs:659`,
`scaffold`).
...
**It checks the host rather than assuming one.** `atlas caps` (`scripts/atlas.mjs:557`) asks which of Wiki,
...
- **An MCP server on stdio** — `atlas mcp` (`scripts/atlas.mjs:758`) exposes seven tools: health, plan,
  ...
  (`atlas mcp --status`).
- **One structured answer with a meaningful exit code** — `atlas ask <task>` (`scripts/atlas.mjs:716`) returns
...
- **Append one record of what was decided or touched** — `atlas note <kind> "<text>"`
  (`scripts/atlas.mjs:599`), with five kinds: decision, finding, trap, progress, blocker
...
- **Know what a fan-out will collide on, before it starts** — `atlas contention`
  (`scripts/atlas.mjs:541`): files more than one branch touches, plan-item ids more than one branch defines,
...
(`scripts/atlas.mjs:719`), anything else is a person's question and takes the document search
(`scripts/atlas.mjs:716`). Verified by running it against this tree:
