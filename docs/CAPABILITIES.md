...
This is the core job. `atlas health` (`scripts/atlas.mjs:1157`) runs sixteen checks over the corpus, reports
...
`atlas scan` (`scripts/atlas.mjs:462`) builds the index; `atlas build` (`scripts/atlas.mjs:1167`) turns it into
...
owes an answer to and marks the result a stub until a human deletes the marker (`scripts/atlas.mjs:651`,
`scaffold`).
...
- **Rebuild while you work** — `atlas watch` polls a fingerprint of every input and rebuilds on change
  (`scripts/atlas.mjs:1613`, the fingerprint poll in the `watch` block).
- **A live dashboard that patches itself** — `atlas serve` builds, starts a loopback server on a port derived
  ...
  rival (`scripts/atlas.mjs:1332`, `scripts/lib/serve.mjs:55`, the adopt-an-existing-server path in the `serve` block).
...
`hooks/on-commit.sh` via `atlas health --gate` at `scripts/atlas.mjs:1137`) and in the commands themselves
...
`atlas publish` (`scripts/atlas.mjs:1186`) stages one of three targets. **Nothing is pushed without an explicit
...
**It checks the host rather than assuming one.** `atlas caps` (`scripts/atlas.mjs:549`) asks which of Wiki,
...
- **An MCP server on stdio** — `atlas mcp` (`scripts/atlas.mjs:750`) exposes seven tools: health, plan,
  ...
  (`atlas mcp --status`).
- **One structured answer with a meaningful exit code** — `atlas ask <task>` (`scripts/atlas.mjs:708`) returns
...
- **Who did what** — `atlas contrib` (`scripts/atlas.mjs:1027`): people, AI co-authors by model, per-desk
...
- **Where the bus factor is one** — `atlas ownership` (`scripts/atlas.mjs:877`).
- **What of the work is still standing** — `atlas surviving` (`scripts/atlas.mjs:870`).
- **The plan, with progress bars** — `atlas tasks [filter]` (`scripts/atlas.mjs:476`).
- **What changed and what it endangers** — `atlas changes` (`scripts/atlas.mjs:778`). The third section is the
...
- **The day's log** — `atlas worklog` (`scripts/atlas.mjs:894`), also refreshed automatically after a build
  (the post-build worklog refresh).
- **Where the tokens went, and how sessions ran** — `atlas tokens` and `atlas sessions`
  (`scripts/atlas.mjs:947`, `scripts/atlas.mjs:991`). They aggregate only and refuse to write into the
...
- **Append one record of what was decided or touched** — `atlas note <kind> "<text>"`
  (`scripts/atlas.mjs:591`), with five kinds: decision, finding, trap, progress, blocker
...
- **Read back what a resuming session needs first** — `atlas state` (`scripts/atlas.mjs:622`): where you are,
...
- **The derived half of a handoff, as a prompt** — `atlas handoff` (`scripts/atlas.mjs:762`). **It never
...
- **Checkpoint everything in flight** — `atlas pause` (`scripts/atlas.mjs:1283`) writes every agent worktree
  to a `wip/agent-*` ref and records the session. `--dry-run` says what it would do and writes nothing.
- **Get the re-spawn plan back** — `atlas resume` (`scripts/atlas.mjs:1283`): branch, worktree and checkpoint
  per agent. **It writes nothing** — it prints what to run, and a person runs it.
- **Clear the session without losing the work** — `atlas stop` (`scripts/atlas.mjs:1283`) removes session
...
- **Know what a fan-out will collide on, before it starts** — `atlas contention`
  (`scripts/atlas.mjs:533`): files more than one branch touches, plan-item ids more than one branch defines,
...
`atlas git-insights [section]` (`scripts/atlas.mjs:1053`, alias `git-insight`) is read-only and derives
...
`atlas git-tree` (`scripts/atlas.mjs:1090`) is the same refs drawn as a shape rather than a table: what was
...
- **Check before you write** — `atlas branch` (`scripts/atlas.mjs:485`) reports where you are and exits
  non-zero when it is not safe to commit there (the non-zero exit in the `branch` block).
- **Propose the route before the decision, not after** — `atlas plan` (`scripts/atlas.mjs:835`) works out
...
- **Two commit gates** — a blocking documentation signal (`scripts/atlas.mjs:1137`) and a commit that names no
  plan item (`scripts/atlas.mjs:1107`).
...
It is **not** the only item below 100%: the plan holds 112 items at a mean of 96.7%, seven of them short of
...
(`scripts/atlas.mjs:711`), anything else is a person's question and takes the document search
(`scripts/atlas.mjs:708`). Verified by running it against this tree:
...
| A `project-atlas.config.json` | Written by `atlas init`. Without one, the hooks are inert and `atlas ask` refuses with exit 2. | `scripts/atlas.mjs:376`, `scripts/lib/task.mjs:74-80` |
