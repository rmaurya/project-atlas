...
This is the core job. `atlas health` (`scripts/atlas.mjs:1177`) runs sixteen checks over the corpus, reports
...
`atlas scan` (`scripts/atlas.mjs:463`) builds the index; `atlas build` (`scripts/atlas.mjs:1207`) turns it into
...
- **The same derived facts as markdown**, under `kb/`, for an agent that has `Read` and `Grep` and no browser
  (`scripts/lib/render.mjs:400`, `scripts/lib/kb.mjs`).
...
owes an answer to and marks the result a stub until a human deletes the marker (`scripts/atlas.mjs:660`,
`scaffold`).
...
- **Rebuild while you work** — `atlas watch` polls a fingerprint of every input and rebuilds on change
  (`scripts/atlas.mjs:1728`, the fingerprint poll in the `watch` block).
- **A live dashboard that patches itself** — `atlas serve` builds, starts a loopback server on a port derived
  ...
  rival (`scripts/atlas.mjs:1394`, `scripts/lib/serve.mjs:55`, the adopt-an-existing-server path in the `serve` block).
...
`hooks/on-commit.sh` via `atlas health --gate` at `scripts/atlas.mjs:1177`) and in the commands themselves
...
`atlas publish` (`scripts/atlas.mjs:1248`) stages one of three targets. **Nothing is pushed without an explicit
...
**It checks the host rather than assuming one.** `atlas caps` (`scripts/atlas.mjs:558`) asks which of Wiki,
...
- **An MCP server on stdio** — `atlas mcp` (`scripts/atlas.mjs:775`) exposes seven tools: health, plan,
  ...
  (`atlas mcp --status`).
- **One structured answer with a meaningful exit code** — `atlas ask <task>` (`scripts/atlas.mjs:717`) returns
...
- **Who did what** — `atlas contrib` (`scripts/atlas.mjs:1067`): people, AI co-authors by model, per-desk
...
- **Where the bus factor is one** — `atlas ownership` (`scripts/atlas.mjs:917`).
- **What of the work is still standing** — `atlas surviving` (`scripts/atlas.mjs:910`).
- **The plan, with progress bars** — `atlas tasks [filter]` (`scripts/atlas.mjs:477`).
- **What changed and what it endangers** — `atlas changes` (`scripts/atlas.mjs:803`). The third section is the
...
- **The day's log** — `atlas worklog` (`scripts/atlas.mjs:934`), also refreshed automatically after a build
  (the post-build worklog refresh).
- **Where the tokens went, and how sessions ran** — `atlas tokens` and `atlas sessions`
  (`scripts/atlas.mjs:987`, `scripts/atlas.mjs:1031`). They aggregate only and refuse to write into the
...
- **Append one record of what was decided or touched** — `atlas note <kind> "<text>"`
  (`scripts/atlas.mjs:600`), with five kinds: decision, finding, trap, progress, blocker
...
- **Read back what a resuming session needs first** — `atlas state` (`scripts/atlas.mjs:631`): where you are,
...
- **The derived half of a handoff, as a prompt** — `atlas handoff` (`scripts/atlas.mjs:787`). **It never
...
- **Checkpoint everything in flight** — `atlas pause` (`scripts/atlas.mjs:1345`) writes every agent worktree
  to a `wip/agent-*` ref and records the session. `--dry-run` says what it would do and writes nothing.
- **Get the re-spawn plan back** — `atlas resume` (`scripts/atlas.mjs:1345`): branch, worktree and checkpoint
  per agent. **It writes nothing** — it prints what to run, and a person runs it.
- **Clear the session without losing the work** — `atlas stop` (`scripts/atlas.mjs:1345`) removes session
...
- **Know what a fan-out will collide on, before it starts** — `atlas contention`
  (`scripts/atlas.mjs:542`): files more than one branch touches, plan-item ids more than one branch defines,
...
`atlas git-insights [section]` (`scripts/atlas.mjs:1093`, alias `git-insight`) is read-only and derives
...
`atlas git-tree` (`scripts/atlas.mjs:1130`) is the same refs drawn as a shape rather than a table: what was
...
**Four refusals worth knowing**, because each is a number the report will not give you:

- **A branch's parent is never printed as a fact.** Git records no parent for a branch, so `git-tree` and the
  branch table both infer one from the commit graph and label the inference. Two branches that each look like
  the other's parent — what "cut a branch, then keep committing on the one you cut it from" leaves behind —
  are reported as one undirected split with neither named, unless this clone's reflog can order them, in
  which case the report says that is where the ordering came from. A branch with no common ancestor at all is
  listed with that reason rather than hung off the trunk to tidy the picture.
...
- **Check before you write** — `atlas branch` (`scripts/atlas.mjs:494`) reports where you are and exits
  non-zero when it is not safe to commit there (the non-zero exit in the `branch` block).
- **Propose the route before the decision, not after** — `atlas plan` (`scripts/atlas.mjs:875`) works out
...
- **Two commit gates** — a blocking documentation signal (`scripts/atlas.mjs:1177`) and a commit that names no
  plan item (`scripts/atlas.mjs:1147`).
...
It is **not** the only item below 100%: the plan holds 122 items at a mean of 97%, seven of them short of
...
(`scripts/atlas.mjs:720`), anything else is a person's question and takes the document search
(`scripts/atlas.mjs:717`). Verified by running it against this tree:
...
| A `project-atlas.config.json` | Written by `atlas init`. Without one, the hooks are inert and `atlas ask` refuses with exit 2. | `scripts/atlas.mjs:377`, `scripts/lib/task.mjs:74-80` |
