...
This is the core job. `atlas health` (`scripts/atlas.mjs:1083`) runs sixteen checks over the corpus, reports
...
`atlas scan` (`scripts/atlas.mjs:380`) builds the index; `atlas build` (`scripts/atlas.mjs:1093`) turns it into
...
owes an answer to and marks the result a stub until a human deletes the marker (`scripts/atlas.mjs:577`,
`scaffold`).
...
- **Rebuild while you work** — `atlas watch` polls a fingerprint of every input and rebuilds on change
  (`scripts/atlas.mjs:1539`, the fingerprint poll in the `watch` block).
- **A live dashboard that patches itself** — `atlas serve` builds, starts a loopback server on a port derived
  ...
  rival (`scripts/atlas.mjs:1258`, `scripts/lib/serve.mjs:55`, the adopt-an-existing-server path in the `serve` block).
...
`hooks/on-commit.sh` via `atlas health --gate` at `scripts/atlas.mjs:1063`) and in the commands themselves
...
`atlas publish` (`scripts/atlas.mjs:1112`) stages one of three targets. **Nothing is pushed without an explicit
...
**It checks the host rather than assuming one.** `atlas caps` (`scripts/atlas.mjs:475`) asks which of Wiki,
...
- **An MCP server on stdio** — `atlas mcp` (`scripts/atlas.mjs:676`) exposes seven tools: health, plan,
  ...
  (`atlas mcp --status`).
- **One structured answer with a meaningful exit code** — `atlas ask <task>` (`scripts/atlas.mjs:634`) returns
...
- **Who did what** — `atlas contrib` (`scripts/atlas.mjs:953`): people, AI co-authors by model, per-desk
...
- **Where the bus factor is one** — `atlas ownership` (`scripts/atlas.mjs:803`).
- **What of the work is still standing** — `atlas surviving` (`scripts/atlas.mjs:796`).
- **The plan, with progress bars** — `atlas tasks [filter]` (`scripts/atlas.mjs:394`).
- **What changed and what it endangers** — `atlas changes` (`scripts/atlas.mjs:704`). The third section is the
...
- **The day's log** — `atlas worklog` (`scripts/atlas.mjs:820`), also refreshed automatically after a build
  (the post-build worklog refresh).
- **Where the tokens went, and how sessions ran** — `atlas tokens` and `atlas sessions`
  (`scripts/atlas.mjs:873`, `scripts/atlas.mjs:917`). They aggregate only and refuse to write into the
...
- **Append one record of what was decided or touched** — `atlas note <kind> "<text>"`
  (`scripts/atlas.mjs:517`), with five kinds: decision, finding, trap, progress, blocker
...
- **Read back what a resuming session needs first** — `atlas state` (`scripts/atlas.mjs:548`): where you are,
...
- **The derived half of a handoff, as a prompt** — `atlas handoff` (`scripts/atlas.mjs:688`). **It never
...
- **Checkpoint everything in flight** — `atlas pause` (`scripts/atlas.mjs:1209`) writes every agent worktree
  to a `wip/agent-*` ref and records the session. `--dry-run` says what it would do and writes nothing.
- **Get the re-spawn plan back** — `atlas resume` (`scripts/atlas.mjs:1209`): branch, worktree and checkpoint
  per agent. **It writes nothing** — it prints what to run, and a person runs it.
- **Clear the session without losing the work** — `atlas stop` (`scripts/atlas.mjs:1209`) removes session
...
- **Know what a fan-out will collide on, before it starts** — `atlas contention`
  (`scripts/atlas.mjs:459`): files more than one branch touches, plan-item ids more than one branch defines,
...
`atlas git-insights [section]` (`scripts/atlas.mjs:979`, alias `git-insight`) is read-only and derives
...
`atlas git-tree` (`scripts/atlas.mjs:1016`) is the same refs drawn as a shape rather than a table: what was
...
- **Check before you write** — `atlas branch` (`scripts/atlas.mjs:411`) reports where you are and exits
  non-zero when it is not safe to commit there (the non-zero exit in the `branch` block).
- **Propose the route before the decision, not after** — `atlas plan` (`scripts/atlas.mjs:761`) works out
...
- **Two commit gates** — a blocking documentation signal (`scripts/atlas.mjs:1063`) and a commit that names no
  plan item (`scripts/atlas.mjs:1033`).
...
(`scripts/atlas.mjs:637`), anything else is a person's question and takes the document search
(`scripts/atlas.mjs:634`). Verified by running it against this tree:
