...
| `atlas branch [type slug]` | Report branch safety, or create `type/short-slug` carrying uncommitted work. | `scripts/atlas.mjs:485` | `/atlas:branch`, `/atlas:git-branch`, `/atlas:git-status` | shipped |
| `atlas contention [branch…]` | What a fan-out will collide on: files more than one branch touches, plan-item ids more than one branch defines, and the next free id. `--base REF`. Exit 1 on a duplicate id only. | `scripts/atlas.mjs:533` | — | shipped |
| `atlas caps` (alias `capabilities`) | Probe which host features are on. Makes a network request, and says so. | `scripts/atlas.mjs:549` | `/atlas:caps` | shipped |
| `atlas community [--write]` | Generate issue/PR/Discussions scaffolding for the features the host supports. | `scripts/atlas.mjs:557` | `/atlas:community` | shipped |
| `atlas note <kind> "<text>"` | Append one record to the continuity journal. **The only writing slash command.** | `scripts/atlas.mjs:591` | `/atlas:note` | shipped |
| `atlas state` | What a resuming session reads first: branch, uncommitted work, journal. | `scripts/atlas.mjs:622` | `/atlas:state` | shipped |
| `atlas design [--scaffold]` | The design record's state; `--scaffold` writes question stubs, never answers. | `scripts/atlas.mjs:651` | `/atlas:design` | shipped |
| `atlas ask <task>` | One structured JSON answer with a meaningful exit code. Takes a **task id**. | `scripts/atlas.mjs:711` | `/atlas:ask` | shipped |
| `atlas ask <question>` | The documents worth reading, for a person. Anything that is not a task id takes this path. | `scripts/atlas.mjs:708`, `scripts/atlas.mjs:803` | `/atlas:ask` | shipped |
| `atlas mcp` | Serve the corpus over MCP on stdio. `--status` reports what a client would connect to. | `scripts/atlas.mjs:750` | `/atlas:mcp` | shipped |
| `atlas handoff` | The derived half of a handoff, printed as a prompt. Writes no file. | `scripts/atlas.mjs:762` | `/atlas:handoff` | shipped |
| `atlas changes` | Uncommitted work, branch-local work, and the documents that cite the files touched. | `scripts/atlas.mjs:778` | `/atlas:changes`, `/atlas:review`, `/atlas:git-status` | shipped |
| `atlas config` | The merged configuration — user overrides shown against the defaults. | `scripts/atlas.mjs:790` | `/atlas:config` | shipped |
| `atlas plan [slug]` | Propose the git route for work in progress. `--apply` creates the branch and nothing else. | `scripts/atlas.mjs:835` | `/atlas:plan` | shipped |
| `atlas surviving` | Surviving lines per file, by author. | `scripts/atlas.mjs:870` | `/atlas:surviving` | shipped |
| `atlas ownership` | Areas by author, and where the bus factor is one. | `scripts/atlas.mjs:877` | `/atlas:ownership`, `/atlas:git-hotspots` | shipped |
| `atlas worklog` | Write the day's log to `worklog/`. `--stdout` prints instead. | `scripts/atlas.mjs:894` | `/atlas:worklog` (via `--stdout`) | shipped |
| `atlas diff [file]` | One file's diff; with no path, lists what there is to ask about. | `scripts/atlas.mjs:912` | `/atlas:diff`, `/atlas:git-diff` | shipped |
| `atlas tokens` | Token accounting from local session transcripts. | `scripts/atlas.mjs:947` | `/atlas:tokens` | shipped |
| `atlas sessions` | Session outcomes — turns, interruptions, compactions, tool friction, rework. | `scripts/atlas.mjs:991` | `/atlas:sessions` | shipped |
| `atlas prompt` | A system prompt assembled from this repository's own config, plan and corpus. | `scripts/atlas.mjs:1007` | `/atlas:prompt` | shipped |
| `atlas contrib` | Contribution analysis from `git log` alone. | `scripts/atlas.mjs:1027` | `/atlas:contrib` | shipped |
| `atlas git-insights [section]` (alias `git-insight`) | What git history says that nothing else here reads: hotspots, coupling, branches, cadence, hygiene, change. Read-only. | `scripts/atlas.mjs:1053` | `/atlas:git-insights`, `/atlas:git-hotspots`, `/atlas:git-history`, `/atlas:git-branch`, `/atlas:git-diff`, `/atlas:git-status` | shipped |
| `atlas git-tree` | Branch topology: what was cut from what, where each split off, what has gone back. Every origin is inferred from the commit graph and marked as such — git records no parent for a branch. Read-only. | `scripts/atlas.mjs:1090` | `/atlas:git-tree` | shipped |
| `atlas spec --gate` | Commit gate: refuse a staged change whose message names no plan item. | `scripts/atlas.mjs:1107` | — | shipped |
| `atlas health --gate` | Commit gate: refuse a commit that would land a blocking signal. | `scripts/atlas.mjs:1137` | — | shipped |
| `atlas health` | The rot report. Exit 1 when any blocking signal fires. | `scripts/atlas.mjs:1157` | `/atlas:health` | shipped |
| `atlas build` | Generate the static site. `--verify` audits what was just written. | `scripts/atlas.mjs:1167` | `/atlas:build` | shipped |
| `atlas all` | `scan` + `health` + `build`; exit 1 if blocking. | `scripts/atlas.mjs:1167`, `scripts/atlas.mjs:1182` | — | shipped |
| `atlas publish --target wiki\|pages\|export` | Stage a publish target. Nothing pushes without `--push`. | `scripts/atlas.mjs:1186` | `/atlas:publish` | shipped |
| `atlas pause [--dry-run]` | Checkpoint every agent worktree to a `wip/agent-*` ref and record the session. | `scripts/atlas.mjs:1283`, `scripts/atlas.mjs:1285` | `/atlas:pause` | shipped |
| `atlas resume` | The re-spawn plan for a paused session — branch, worktree, checkpoint. Writes nothing. | `scripts/atlas.mjs:1283`, `scripts/atlas.mjs:1297` | `/atlas:resume` | shipped |
| `atlas stop [--force]` | Clear session state and agent worktrees; every branch and checkpoint survives. | `scripts/atlas.mjs:1283` | `/atlas:stop` | shipped |
| `atlas serve` | Start (or adopt) the live dashboard server and open it. | `scripts/atlas.mjs:1332` | `/atlas:dashboard` | shipped |
| `atlas serve --stop \| --status \| --list \| --launcher` | Stop it, report it, list every dashboard on the machine, or write a launcher page. | `scripts/atlas.mjs:1332`, then `--stop`, `--status`, `--list`, `--launcher` | `/atlas:dashboard` (`--list`) | shipped |
...
| `atlas watch [--serve]` | Rebuild on change; `--serve` hosts the output on loopback. | `scripts/atlas.mjs:1613` | — | shipped |
